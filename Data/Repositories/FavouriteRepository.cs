...
public class FavouriteRepository(MusicDbContext context) : IFavouriteRepository
{
    ...
    {
        var existing = await context.UserArtists
            .AnyAsync(ua => ua.UserId == userId && ua.ArtistId == artistId);

        if (!existing)
        {
            await context.UserArtists.AddAsync(new UserArtist
            {
                UserId = userId,
                ArtistId = artistId
            });
    ...
    public async Task<HashSet<int>> GetFavouriteAlbumsIds(int userId)
    {
        return await context.UserAlbums
            .Where(ua => ua.UserId == userId)
            .Select(ua => ua.AlbumId).ToHashSetAsync();
    }
